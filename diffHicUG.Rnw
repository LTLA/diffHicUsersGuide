...
\section{Scope}
This document describes the analysis of Hi-C data with the \Biocpkg{diffHic} package.
Differential interactions are defined as those with significant changes in intensity between conditions.
These are identified in a statistically rigorous manner using the methods in the \Biocpkg{edgeR} package \cite{edgeR}.
...
The first dataset is the smallest and examines the chromatin structure in K562 and GM06990 cell lines \cite{lieberman2009comprehensive}.
...
The ligation signature in each chimeric read is identified with cutadapt \cite{martin2011cutadapt}, and the read is split into two segments at the center of the signature.
Each segment is then aligned separately to the reference genome using Bowtie2 \cite{langmead2012bowtie}.
Mapping by read splitting can be performed in \Biocpkg{diffHic} using a custom Python script, below.
...
For \Rfunction{preparePairs}, chimeric reads are handled by recording two separate alignments for each read.
...
preparePairs("SRR027957.bam", pairParam(seg.frags), 
...
Instead, alignment programs should be used that can directly handle chimeric reads with arbitrary breakpoints in the genome, e.g., BWA \cite{li2010fast}.
...
The object also contains a count matrix with the number of read pairs for each interaction in each library.
...
Users can elect to restrict counting to particular chromosomes, by setting a value for the \Rcode{restrict} slot in the \Rclass{pairParam} object.
...
Users can also choose to discard alignments that lie within blacklisted regions, using the \Rcode{discard} slot.
...
The first task is to load the counts for the smaller bin pairs, using a reasonably large \Rcode{filter} to avoid excessive memory consumption.
...
Direct filtering of these smaller bin pairs can be performed using \Rfunction{filterDirect}, with assistance from their larger counterparts.
Counts for the 1 Mbp bin pairs are supplied as the \Rcode{reference} parameter and are used to estimate the filter threshold.
...
The normalization factors can be stored in the \Rcode{colData} of the \Rclass{InteractionSet} object.
...
When fitting a generalized linear model (GLM), a large offset for a count is equivalent to downscaling that count relative to the counts of other libraries. 
The matrix of offsets is of the same dimension as the count matrix and is stored as an element of the \Rcode{assays} slot of the \Rclass{InteractionSet} object.
(Setting \Rcode{se.out=FALSE} returns the offset matrix directly, instead of a modified \Rclass{InteractionSet} object containing the offsets.)
...
nb.off[!neardiag] <- normOffsets(data[!neardiag,], type="loess", se.out=FALSE)
assay(data, "offset") <- nb.off # updating offset matrix
@
...
\label{sec:itercor}
While this is not the intended function of the \Biocpkg{diffHic} package, a method is also provided for the removal of biases between genomic regions.
The \Rfunction{correctedContact} function performs iterative correction \cite{imakaev2012iterative} with some modifications.
Briefly, if multiple libraries are used to generate the \Rcode{data} object, then correction is performed using the overall NB mean for each bin pair.
...
This is done using the \Rfunction{normalizeCNV} function, based on multi-dimensional smoothing across several covariates with the \CRANpkg{locfit} package \cite{loader1999local}.
It requires both bin pair and marginal counts, obtained with the same values of \Rcode{width} and \Rcode{param} in \Rfunction{squareCounts} and \Rfunction{marginCounts}, respectively.
...
We generate some diagnostic plots to determine the effect of \Rfunction{normalizeCNV}.
...
Note that increasing \Rcode{maxk} may be necessary to obtain sufficient accuracy in the internal call to \CRANpkg{locfit}.
...
Specifically, variability is modelled in \Biocpkg{edgeR} by estimating the NB dispersion parameter.
This is used during testing to reduce the significance of any detected differences when the counts are highly variable.
Similarly, estimation of the quasi-likelihood (QL) dispersion can be performed to account for heteroskedasticity \cite{lund2012ql}. 
...
The \Rclass{InteractionSet} object is also converted into a \Rclass{DGEList} object for analysis with \Biocpkg{edgeR}.
If the normalization factors or offsets are already stored in certain fields of \Robject{data}, as previously described, then they will be automatically extracted and used by \Rfunction{asDGEList}.
Alternatively, they can be passed to \Rfunction{asDGEList} explicitly.
...
Estimation of the former is performed with the \Rfunction{glmQLFit} function.
...
Users should check that the contrast has been specified correctly through the \Rcode{coef} or \Rcode{contrast} arguments.
...
To mitigate this effect, \Biocpkg{diffHic} provides several methods for controlling the FDR.
...
Specifically, the cluster-level FDR is estimated from the bin pair-level FDR threshold used to define significant windows (see the \Rfunction{clusterFDR} function in the \Biocpkg{csaw} package).
...
Additional details on the bin pairs in each cluster can be obtained using the \Rfunction{combineTests} and \Rfunction{getBestTest} functions in \Biocpkg{csaw}.
...
Both \Rfunction{diClusters} and \Rfunction{clusterPairs} accept multiple \Rclass{InteractionSet} objects to cluster results of analyses with different sizes.
...
(Similarly, \Rfunction{diClusters} uses a frequency-weighted version of the FDR to ensure an equal contribution from each bin size when defining significant bin pairs.)
...
Plaid plots can be used to visualize the distribution of read pairs in the interaction space \cite{lieberman2009comprehensive}. 
...
However, this analysis is largely outside the scope of \Biocpkg{diffHic} and will not be discussed in detail here.
...
If the directionality was positive in the WT cells, a positive \Rcode{logFC} would represent a strengthening of the domain boundary.
However, if the WT directionality was negative, a positive \Rcode{logFC} would represent a weakening of the domain boundary or a reversal of the domain orientation.
