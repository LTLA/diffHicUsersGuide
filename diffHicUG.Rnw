...
Counting of read pairs between paired bins is performed for multiple libraries using the \code{squareCounts} function.
...
This generates an \code{InteractionSet} object containing information for multiple genomic interactions \citep{lun2016infrastructure}.
...
It is possible to use counts from existing count matrices, by using coercion methods available to \code{ContactMatrix} objects from the InteractionSet package \cite{lun2016infrastructure}.
Assume that there are two \code{ContactMatrix} objects (\code{cm1} and \code{cm2}) spanning the same region of the binned interaction space.
The following code is used to convert these two objects into a single \code{InteractionSet} object.
Only bin pairs with a count sum greater than 10 are retained, analogous to the output obtained with a \code{squareCounts} call.
This facilitates input from other Hi-C data processing pipelines that return count matrices rather than BAM files.
...
\setkeys{Gin}{width=0.8\textwidth}
\begin{center}
<<echo=FALSE,fig.width=10,fig.height=6,resolution=100>>=
<<rot1>>
@ 
\\
<<echo=FALSE,fig.width=10,fig.height=6,resolution=100>>=
