...
The statistics can also stored in the \code{rowData} of the \code{GInteractions} object returned by \code{clusterPairs}. 
This ensures that the genomic coordinates and statistics are synchronised.

<<>>=
rowData(clustered.small$interactions) <- cbind(
    rowData(clustered.small$interactions), tabcluster)
@
...
Again, the statistics can stored in the \code{rowData} of \code{GInteractions} object.

<<>>=
rowData(clustered.sig$interactions) <- cbind(rowData(clustered.sig$interactions), tabstat)
@
